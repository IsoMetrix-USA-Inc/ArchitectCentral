...
namespace ArchitectCentral.AgentStudio.Services.Query
{
    ...
    public class HybridSearchService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // Stage 1: Vector Similarity Search (each method opens its own connection)
        ...
        private string BuildVectorSearchQuery(SearchFilters? filters)
        {
            ...
            // Use V1 schema (current database)
        ...
        private void AddFilterConditions(List<string> conditions, SearchFilters? filters)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filters.DocType))
                conditions.Add("doc_type = @doc_type::text");

            // NOTE: The following filters are only available in V2 schema
            // Commenting out until database is migrated to V2
        ...
        private static void AddFilterParameters(NpgsqlCommand cmd, SearchFilters? filters)
        {
            ...
            // NOTE: The following parameters are only available in V2 schema
            // Commenting out until database is migrated to V2
